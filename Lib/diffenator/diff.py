...
class DiffFonts:
    ...
    def attribs(self):
        self._data["attribs"] = diff_attribs(self._font_before, self._font_after)

    def names(self):
        self._data["names"] = diff_nametable(self._font_before, self._font_after)
...
def diff_nametable(font_before, font_after):
    ...
    DiffTable
    """
    nametable_a = font_before.names
    nametable_b = font_after.names
...
@timer
def diff_glyphs(font_before, font_after, thresh=0.00, scale_upms=True, render_diffs=False):
    ...
    """
    glyphs_a = font_before.glyphs
    glyphs_b = font_after.glyphs
...
@timer
def diff_marks(font_before, font_after, marks_a, marks_b,
